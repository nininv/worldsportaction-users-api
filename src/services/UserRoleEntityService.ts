...
export default class UserRoleEntityService extends BaseService<UserRoleEntity> {
  ...
  public async DeleteUre(entityId: number, userId: number, currentUserId: number) {
    return this.entityManager
      ...
      .andWhere(
        'userRoleEntity.entityId = :entityId and userRoleEntity.userId = :userId and entityTypeId = 2',
  ...
  public async deleteImpersonationUre(entityId: number, userId: number, currentUserId: number) {
    return this.entityManager
      ...
      .set({ isDeleted: 1, updatedBy: currentUserId, updatedAt: new Date() })
      .andWhere(
        'userRoleEntity.entityId = :entityId and userRoleEntity.userId = :userId and userRoleEntity.roleId = 10',
        { entityId: entityId, userId: userId },
      )
      .execute();
  }
  ...
  public async findByTemplateId(entityId: number): Promise<UserRoleEntity[]> {
    ...
    query
      .select(['ure.userId', 'ure.id'])
      .where(' ure.entityId= :entityId and ure.entityTypeId = 2 and ure.isDeleted = 0 ', {
        entityId,
      });
  ...
  public async findByUserAndEntityId(userId, entityId) {
    ...
    query
      .select(['ure.id'])
      .where(
        ' ure.userId= :userId and ure.entityId= :entityId and ure.entityTypeId = 2 and ure.isDeleted = 0 ',
        { userId, entityId },
      );
  ...
  public async findByParams(
    ...
    if (isArrayPopulated(linkedEntities)) {
      ...
      query.andWhere('ure.entityId in (:entityIds) and ' + 'ure.entityTypeId in (:entityTypeIds)', {
        entityIds: entityIds,
  ...
  public async findExisting(
    ...
    try {
      let query = await this.entityManager
        .createQueryBuilder(UserRoleEntity, 'ure')
        .where(
          'ure.userId = :userId and ure.entityId = :entityId and ure.entityTypeId= :entityTypeId and ure.roleId = :roleId and ure.isDeleted = 0',
