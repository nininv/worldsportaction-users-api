...
export default class UserDashboardService extends BaseService<User> {
  ...
  public async userDashboardTextualList(
    requestBody: any,
    userId: any,
    ...
    try {
      ...
      let result = await this.entityManager.query(
        'call wsa_users.usp_user_dashboard_textual(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
        [
          organisationId,
          yearRefId,
      ...
      if (result != null) {
        ...
        if (result[0] != null && result[0] != undefined) {
          for (let textual of result[0]) {
            ...
            let competitionObj = {
              competitionName:
                textual.competitionName != null ? textual.competitionName.toString() : null,
        ...
        if (isArrayPopulated(result[5])) {
          ...
          let totalUserCount =
            (userCount != null ? Number(userCount.counts) : 0) +
            (userCount1 != null ? Number(userCount1.counts) : 0);
          responseObject['counts']['noOfUsers'] = totalCount;
          // responseObject["counts"]["noOfRegisteredUsers"] = result[6][0].regCount;
          responseObject['counts']['noOfRegisteredUsers'] =
            userCount1 != null ? Number(userCount1.counts) : 0;
        }
  ...
  public async userDashboardTextualSpectatorCount(requestBody: any, userId: any) {
    try {
      ...
      const result = await this.entityManager.query(
        'call wsa_users.usp_user_dashboard_spectator_count(?)',
        [organisationId],
      );
      if (!!result) {
        return {
  ...
  public async exportRegistrationQuestions(requestBody: any, userId) {
    try {
      let result = await this.entityManager.query(
        'call wsa_users.usp_export_registration_questions(?,?,?,?,?,?,?,?)',
        [
          requestBody.organisationId,
          requestBody.yearRefId,
          requestBody.competitionUniqueKey,
          requestBody.roleId,
          requestBody.genderRefId,
          requestBody.linkedEntityId,
