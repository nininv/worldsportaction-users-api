...
export default class OrganisationService extends BaseService<Organisation> {
  ...
  public async getOrganisationByName(name: string) {
    let result = await this.entityManager.query(
      `select o.name, o.id, o.organisationTypeRefId, o.suburb, o.postcode, json_object('name', p.name, 'id', p.id, 'suburb', p.suburb, 'postcode', p.postalCode) parent from wsa_users.organisation o
