...
export class TermsAndConditionsService extends BaseService<TC> {
  ...
  async createForOrganisation(organisationId: number, body: TC): Promise<TC> {
  ...
  async getAcknowledgementListForUser(user: User, tcType: TCTypeEnum): Promise<any> {
    ...
    return Promise.all(
      (await this.entityManager.find(TC, conditions)).map(async tc => {
  ...
  async batchAcknowledge(user: User, tcIds: number[]): Promise<TC[]> {
    ...
    for (const tcId of tcIds) {
      ...
      const acknowledgement =
        (await this.entityManager.findOne(TCUserAcknowledgement, acknowledgementData)) ||
        (await this.entityManager.create(TCUserAcknowledgement, acknowledgementData));
